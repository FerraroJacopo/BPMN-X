...
ReplaceMenuProvider.prototype.getHeaderEntries = function(element) {
  ...
  if (is(element, 'bpmn:DataStoreReference')) {
    headerEntries = headerEntries.concat(this._getDataStoreCollection(element));
  }
...
ReplaceMenuProvider.prototype._getDataObjectIsCollection = function(element) {
  ...
  function toggleIsCollection(event, entry) {
    self._modeling.updateModdleProperties(
      element,
      dataObject,
      { isCollection: !entry.input });

  }

  function toggleIsInput(event, entry) {



    var dataObject = element.businessObject;

  //alternative inherits provenance of data

 
  if(dataObject.boss==1) {

              var elements = elementRegistry.filter(function(element) {
                 if(is(element, 'bpmn:DataObjectReference') || is(element, 'bpmn:DataStoreReference')) {


                  if(getBusinessObject(element).fatherr!==undefined && getBusinessObject(element).fatherr!=1) {

                   if(getBusinessObject(element).boss.id==dataObject.id) {
  ...
  function toggleIsOutput(event, entry) {
  ...
  function noAlternative(event,entry) {
  ...
  function recovery(event,entry) {

    var businessObject = getBusinessObject(element);

    if(businessObject.recoverable == 0) {

              businessObject.recoverable = 1; 
          ...
          if(businessObject.boss==1) {
              ...
              var elements = elementRegistry.filter(function(element) {
                 if(is(element, 'bpmn:DataObjectReference')) {
                  ...
                  if(getBusinessObject(element).fatherr!==undefined && getBusinessObject(element).fatherr!=1) {
                   ...
                   if(getBusinessObject(element).boss.id==businessObject.id) {

                      getBusinessObject(element).recoverable = 1;
              ...
              else { businessObject.recoverable = 0; 
                 ...
                 if(is(element, 'bpmn:DataObjectReference')) {
                  ...
                  if(getBusinessObject(element).fatherr!==undefined && getBusinessObject(element).fatherr!=1) {
                   ...
                   if(getBusinessObject(element).boss.id==businessObject.id) {
                       ...
                       getBusinessObject(element).recoverable = 0;
  ...
  /*var dataObject2 = element.businessObject.dataObjectRef,
     isInput = dataObject.isInput;

   var dataObject3 = element.businessObject.dataObjectRef,
     isOutput = dataObject.isOutput;*/

var dataObjectEntries = [];

if(element.businessObject.boss==1 || element.businessObject.boss===undefined) {
  ...
  dataObjectEntries = [
    ...
    },
    {
      ...
      active: toggleIsInput,
      action: toggleIsInput,
    },
    {
      id: 'toggle-is-output',
      className: 'bpmn-icon-data-output',
      title: translate('Output'),
      active: toggleIsOutput,
      action: toggleIsOutput,
    }
  ]; 

  if(element.businessObject.available==1) {

    dataObjectEntries.push({
      id: 'toggle-recovery',
      className: 'bpmn-icon-recoverable',
      title: translate('Reovery'),
      active: recovery,
      action: recovery
    });
...
if(element.businessObject.available==1 && !is(element.businessObject.child,'bpmn:DataObjectReference')) {

  dataObjectEntries.push({
      id: 'toggle-no-alternative',
      className: 'bpmn-icon-x',
      title: translate('No Alternative'),
      active: noAlternative,
      action: noAlternative
    });
...
dataObjectEntries = [
  ...
  if(element.businessObject.available==1 && is(element.businessObject.fatherr,'bpmn:DataObjectReference') && !is(element.businessObject.child,'bpmn:DataObjectReference')) {
...
ReplaceMenuProvider.prototype._getDataStoreCollection = function(element) {
...
if(element.businessObject.available==1 && !is(element.businessObject.child,'bpmn:DataStoreReference')) {

  dataObjectEntries.push({
      id: 'toggle-no-alternativee',
      className: 'bpmn-icon-x',
      title: translate('No Alternative'),
      active: noAlternative,
      action: noAlternative
...
if(element.businessObject.available==1) {
  ...
  if(element.businessObject.available==1 && is(element.businessObject.fatherr,'bpmn:DataStoreReference') && !is(element.businessObject.child,'bpmn:DataStoreReference')) {
