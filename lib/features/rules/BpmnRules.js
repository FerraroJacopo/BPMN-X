...
function canConnectDataAssociation(source, target) {
  ...
  if (isAny(source, [ 'bpmn:DataObjectReference', 'bpmn:DataStoreReference' ]) &&
      isAny(target, [ 'bpmn:Activity', 'bpmn:ThrowEvent' ]) && !(is(getBusinessObject(source).boss, [ 'bpmn:DataObjectReference'])) && !(is(getBusinessObject(source).boss, [ 'bpmn:DataStoreReference']))) {
    return { type: 'bpmn:DataInputAssociation' };
  }

  if (isAny(target, [ 'bpmn:DataObjectReference', 'bpmn:DataStoreReference' ]) &&
      isAny(source, [ 'bpmn:Activity', 'bpmn:CatchEvent' ]) && !(is(getBusinessObject(target).boss, [ 'bpmn:DataObjectReference'])) && !(is(getBusinessObject(target).boss, [ 'bpmn:DataStoreReference']))) { 
    return { type: 'bpmn:DataOutputAssociation' };
  }


  //CUSTOM
  if (isAny(target, [ 'bpmn:DataObjectReference']) &&
      isAny(source, [ 'bpmn:DataObjectReference' ]) && target!=source ){ 
    return { type: 'bpmn:Association' };
  }


  if (isAny(target, [ 'bpmn:DataStoreReference']) &&
      isAny(source, [ 'bpmn:DataStoreReference' ]) && target!=source ){ 
    return { type: 'bpmn:Association' };
  }
